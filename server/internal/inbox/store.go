...
const itemColumns = `id, question, times_asked, status, last_conversation_id, answer_source_id, last_asked_at, created_at`

func scanItem(row pgx.CollectableRow) (Item, error) {
	var i Item
	err := row.Scan(&i.ID, &i.Question, &i.TimesAsked, &i.Status, &i.LastConversationID,
		&i.AnswerSourceID, &i.LastAskedAt, &i.CreatedAt)
	return i, err
}
...
func (s *Store) List(ctx context.Context, botID, status string, days int) ([]Item, error) {
	rows, err := s.db.Query(ctx, `
		select `+itemColumns+` from inbox_items
		where bot_id = $1 and status = $2 and ($3 = 0 or last_asked_at > now() - make_interval(days => $3))
		order by last_asked_at desc limit 500`, botID, status, days)
	if err != nil {
		return nil, err
	}
...
func (s *Store) Get(ctx context.Context, botID, id string, days int) (*Item, error) {
	rows, err := s.db.Query(ctx, `
		select `+itemColumns+` from inbox_items
		where id = $1 and bot_id = $2 and ($3 = 0 or last_asked_at > now() - make_interval(days => $3))`,
		id, botID, days)
	if err != nil {
		return nil, err
	}
...
func (s *Store) SetStatus(ctx context.Context, botID, id, status string, sourceID *string) (*Item, error) {
	rows, err := s.db.Query(ctx, `
		update inbox_items set status = $3, answer_source_id = coalesce($4, answer_source_id)
		where id = $1 and bot_id = $2 returning `+itemColumns, id, botID, status, sourceID)
	if err != nil {
		return nil, err
	}
	item, err := pgx.CollectExactlyOneRow(rows, scanItem)
